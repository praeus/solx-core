...
//!
//! `get-env` / `set-env` read and write a namespaced `RwLock<HashMap>`,
...
// startup), and anything subsequently written by `set-env`. No relation to
...
// `action-exec`).
//
// Writes are memory-only by default. `set-env` with `persist: true` also
...
// the individual write, so a later `set-env` on an already-persisted key
...
/// makes persistence sticky without re-reading config on every `set-env`.
