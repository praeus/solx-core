...
impl sol::actions::widget::Host for HostState {
    async fn open(&mut self, spec: String) -> std::result::Result<String, String> {
        let spec: Value = serde_json::from_str(&spec).map_err(|e| format!("invalid spec: {e}"))?;
        let bin_name = spec
            .get("bin_name")
            .and_then(Value::as_str)
            .ok_or_else(|| "spec missing bin_name".to_string())?;
        let tag_name = spec
            .get("tag_name")
            .and_then(Value::as_str)
            .ok_or_else(|| "spec missing tag_name".to_string())?;
        let fields = spec.get("fields").cloned().unwrap_or(Value::Null);

        let config = self.actions.config();
        let descriptor = crate::loopback::widget::open(
            &self.files,
            bin_name,
            tag_name,
            fields,
            self.caller.action_ref(),
            self.caller.invocation_id(),
            std::time::Duration::from_secs(config.widget_connect_ttl_secs()),
            std::time::Duration::from_secs(config.widget_reconnect_grace_secs()),
        )
        .await?;
        serde_json::to_string(&descriptor).map_err(|e| format!("failed to serialize descriptor: {e}"))
    }

    async fn close(&mut self, widget_id: String) -> std::result::Result<(), String> {
        crate::loopback::widget::close(&widget_id).await
    }

    async fn show(&mut self, widget_id: String) -> std::result::Result<(), String> {
        crate::loopback::widget::show(&widget_id).await
    }

    async fn hide(&mut self, widget_id: String) -> std::result::Result<(), String> {
        crate::loopback::widget::hide(&widget_id).await
    }

    async fn get(&mut self, widget_id: String, field: String) -> std::result::Result<String, String> {
        let value = crate::loopback::widget::get(&widget_id, &field).await?;
        serde_json::to_string(&value).map_err(|e| format!("failed to serialize value: {e}"))
    }

    async fn set(&mut self, widget_id: String, field: String, value: String) -> std::result::Result<(), String> {
        let value: Value = serde_json::from_str(&value).map_err(|e| format!("invalid value: {e}"))?;
        crate::loopback::widget::set(&widget_id, &field, value).await
    }

    async fn exec(&mut self, widget_id: String, event: String, payload: String) -> std::result::Result<(), String> {
        let payload: Value = serde_json::from_str(&payload).map_err(|e| format!("invalid payload: {e}"))?;
        crate::loopback::widget::exec(&widget_id, &event, payload).await
    }
}
