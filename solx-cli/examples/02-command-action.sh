...
out=$(solx save action /demo/actions/echo-number --json '{"action_type":"command","fn_name":"echo 42"}')
assert_eq "$(jget "$out" action_type)" "command" "save action: action_type is command"
assert_eq "$(jget "$out" fn_name)" "echo 42" "save action: fn_name stored verbatim"
...
solx save action /demo/actions/echo-params --json '{"action_type":"command","fn_name":"more"}' >/dev/null
...
solx save action /demo/actions/always-fails --json '{"action_type":"command","fn_name":"exit 1"}' >/dev/null
