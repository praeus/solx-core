...
out=$(solx exec /builtin/document/entity_save_document \
...
out=$(solx exec /builtin/document/entity_get_document --json '{"path":"/scratch","name":"note"}')
assert_eq "$(jget "$out" result.contents.n)" "1" "entity_get_document: contents round-trip"

out=$(solx exec /builtin/document/entity_save_document --json '{"path":"/scratch","name":"note","type_ref":"/types/docs/Document","contents":{"n":2}}')
assert_eq "$(jget "$out" result.contents.n)" "2" "entity_save_document: update (upsert) replaces contents"

out=$(solx exec /builtin/document/entity_list_documents --json '{"path_prefix":"/scratch"}')
assert_eq "$(jget "$out" result.items.0.name)" "note" "entity_list_documents: filters by path prefix"

out=$(solx exec /builtin/document/set_field --json '{"path":"/scratch","name":"note","field":"status","value":"reviewed"}')
assert_true "$(jget "$out" success)" "set_field: writes into contents"
out=$(solx exec /builtin/document/get_field --json '{"path":"/scratch","name":"note","field":"status"}')
assert_eq "$(jget "$out" result)" "reviewed" "get_field: reads the field back"

out=$(solx exec /builtin/document/entity_delete_document --json '{"path":"/scratch","name":"note"}')
assert_true "$(jget "$out" success)" "entity_delete_document: succeeds"

# -- general-purpose file store (unrestricted rel_path access) --------------
out=$(solx exec /builtin/file/file_put --json '{"rel_path":"scratch/demo.txt","content":"hello from a builtin action"}')
assert_true "$(jget "$out" success)" "file_put: succeeds"

out=$(solx exec /builtin/file/file_get --json '{"rel_path":"scratch/demo.txt"}')
assert_eq "$(jget "$out" result.content)" "hello from a builtin action" "file_get: reads back the same bytes"

out=$(solx exec /builtin/file/file_copy --json '{"source":"scratch/demo.txt","dest":"scratch/demo-copy.txt"}')
assert_true "$(jget "$out" success)" "file_copy: succeeds"
out=$(solx exec /builtin/file/file_get --json '{"rel_path":"scratch/demo-copy.txt"}')
assert_eq "$(jget "$out" result.content)" "hello from a builtin action" "file_copy: copy has identical contents"

out=$(solx exec /builtin/file/file_delete --json '{"rel_path":"scratch/demo.txt"}')
...
# /builtin/secrets/get_secret and /builtin/secrets/set_secret rows themselves, so both need
...
solx save action /builtin/secrets/get_secret --json "{\"action_config\":{\"secrets\":{\"$secret_name\":\"$key_b64\"}}}" >/dev/null
solx save action /builtin/secrets/set_secret --json "{\"action_config\":{\"secrets\":{\"$secret_name\":\"$key_b64\"}}}" >/dev/null

out=$(solx exec /builtin/secrets/set_secret --json "{\"name\":\"$secret_name\",\"value\":\"correct horse battery staple\"}")
assert_true "$(jget "$out" success)" "set_secret: encrypts and stores under the action's own key"

out=$(solx exec /builtin/secrets/get_secret --json "{\"name\":\"$secret_name\"}")
...
out=$(solx script -e "exec /builtin/env/set_env --json '{\"key\":\"SOLX_EXAMPLE_ENV\",\"value\":\"round-tripped\"}'; exec /builtin/env/get_env --json '{\"key\":\"SOLX_EXAMPLE_ENV\"}'")
