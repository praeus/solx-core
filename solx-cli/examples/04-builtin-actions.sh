...
out=$(solx exec /builtin/document/entity_save_document --json '{"path":"/scratch","name":"note","typeRef":"/types/docs/Document","contents":{"n":2}}')
assert_eq "$(jget "$out" result.contents.n)" "2" "entity_save_document: update (upsert) replaces contents"

out=$(solx exec /builtin/document/entity_list_documents --json '{"pathPrefix":"/scratch"}')
...
solx save action /builtin/secrets/get_secret --json "{\"actionConfig\":{\"secrets\":{\"$secret_name\":\"$key_b64\"}}}" >/dev/null
solx save action /builtin/secrets/set_secret --json "{\"actionConfig\":{\"secrets\":{\"$secret_name\":\"$key_b64\"}}}" >/dev/null
