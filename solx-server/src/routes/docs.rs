...
async fn save_doc(
    ...
    Json(req): Json<SaveRequest<solx_surface::entities::DocumentInput>>,
) -> Result<Json<Document>, ApiError> {
    let doc = state.app.docs().save(&req.path, &req.name, req.input).await?;
    Ok(Json(doc))
}
